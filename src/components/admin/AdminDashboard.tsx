...
// Helper to extract YouTube Video ID from any YouTube URL or direct ID
export const extractYoutubeId = (urlOrId: string): string => {
  ...
  // If user pasted an 11-char ID directly
...
// Helper to extract YouTube Playlist ID from any playlist URL
...
// Smart parser for YouTube video title & singer
export const parseYoutubeMeta = (rawTitle: string, authorName: string, videoId: string) => {
  let title = rawTitle.trim();
  let singer = authorName.trim() || 'पारंपरिक लोक गायक';

  // Remove common generic video junk from title like (Official Video), [4K], Full Audio, etc.
  title = title
    .replace(/(\(|\[)(official\s*(video|audio)?|4k|hd|lyric(s)?|full\s*song|video\s*song|audio\s*song|remaster(ed)?)(\)|\])/gi, '')
    .trim();

  // If title has separators like '|', '-', ':', split into Title and Singer
  if (title.includes('|')) {
    const parts = title.split('|').map(p => p.trim()).filter(Boolean);
    if (parts.length >= 2) {
      title = parts[0];
      // Often second part is singer name
  ...
  // Detect category
  let category: Song['category'] = 'Traditional';
  ...
  return { title, singer, thumbnail, category, language };
};
